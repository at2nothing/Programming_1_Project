...
/**
* @brief Main function of the program. Prints welcome message and prompts user for choice. Calls new_account () or ATM () depending on user choice.
* @return Exit code ( 0 means success non - zero means failure ). In case of error the program exits
*/
int main() {
...
/**
* @brief Returns the number of accounts that have been created. This is used to determine how many accounts will be created in the case of an error or when a user tries to create more than one account at a time.
* @return The number of accounts that have been created or - 1 if there are no accounts in the account vector
*/
int account_num_creation() {
...
/**
* @brief Create a new account in the database and add it to the accounts_vector. This is a function that should be called every time the user enters an account
*/
void new_account() {
    Account account;
    print(
    ...
    account.balance = check_int_input(0, 1000000000);
    cout << endl;
...
void ATM() {
    ...
    if (accountIterator == accounts_vector.end()) {
        ...
        while (attempts > 0) {
            ...
            // Perform a transaction if the user entered the password.
...
/**
* @brief Performs the transaction and prompts the user to choose. This is a blocking function. It returns when the user confirms or exits
...
/**
* @brief Withdraws funds from the account. This function is called by the program when the user presses the withdraw button.
* @param account Reference to the account to withdraw from. This is passed by reference
*/
void performWithdrawal(Account &account) {
    ...
    // Withdrawal is successful and the account has been withdrawal.
...
/**
* @brief Performs deposit and updates the balance. This is a callback function that will be called by the user when they want to deposit a new amount.
...
/**
* @brief Transfer funds from one account to another. This is a function that does not return until the transfer is complete
* @param sourceAccount The account to transfer
*/
void performFundTransfer(Account &sourceAccount) {
    ...
    // Transfer the amount of funds from the source account to the target account.
    if (targetAccountIterator != accounts_vector.end()) {
        ...
        // This method is used to transfer the funds from the source account to the target account.
...
/**
* @brief Prints information about an account. This is used to determine if we are running in Gold or Silver mode
...
/**
* @brief Prints a string to the screen. This is a debugging function to be used in conjunction with print_string
...
/**
* @brief \ fn format_on \ brief Turn on formating \ param [ in ] none \ param [ out ]
*/
void format_on() {
...
/**
* @brief Check passcode for validity and return number of passcodes. Print error if invalid passcode. Return value is 0 for success or non - zero for failure.
* @return number of passcodes in string or - 1 for failure ( check_passcode () is called by this
*/
int check_passcode() {
    string passcode;
    cin >> passcode;
...
/**
* @brief Checks if user input is between min and max. If not prints error to user and try again.
* @param min Minimum value to check. This is the minimum allowed value.
* @param max Maximum value to check. This is the maximum allowed value.
* @return Returns the input value or - 1 if user enters invalid input ( in which case error is printed
*/
int check_int_input(int min, int max) {
    int input;
    cin >> input;
